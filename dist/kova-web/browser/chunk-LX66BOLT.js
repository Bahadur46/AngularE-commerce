...
var OCCASIONS = [
  {
    ...
    items: [
      ...
      [
        "Naming Day Keepsake Tray",
        1899,
        2299,
        4.8,
        22,
        9,
        "1 engraved wooden name plaque|1 ink-print kit, hand and foot|1 photo frame, 5\xD77|1 velvet-lined tray|1 hand-written card"
      ],
      [
        "Little One Gift Basket",
        749,
        999,
        4.5,
        66,
  ...
  },
  {
    ...
    name: "Birthday Gift",
    slug: "birthday-gift",
    brand: "Anuvesha & Co. Celebrations",
    tint: 3,
  ...
  },
  {
    ...
    items: [
      ...
      [
        "Couple Keepsake Curation",
        1999,
        2399,
        4.7,
        43,
        15,
        "2 engraved brass tumblers|2 scented candles, 120 g each|1 photo frame, 6\xD78|12 chocolates|1 hand-written card"
      ],
      [
        "Bridal Sringar Trunk",
        3499,
        3999,
        4.9,
        28,
      ...
      [
        "Mehendi Favour Boxes, Set of Ten",
        1499,
        1799,
        4.6,
        51,
  ...
  },
  {
    ...
    items: [
      [
        "Anniversary Keepsake Pair",
        1499,
        1899,
        4.7,
        109,
        26,
        "2 engraved keepsake coasters|1 scented candle, 180 g|1 photo frame, 5\xD77|9 chocolates|1 card"
      ],
      [
        "Golden Jubilee Curation",
        2999,
        3499,
        4.9,
        19,
...
var CATALOGUE_CATEGORIES = OCCASIONS.map((o, index) => ({
  id: o.id,
  name: o.name,
  slug: o.slug,
...
var CATALOGUE = OCCASIONS.flatMap((o, oi) => o.items.map(([name, price, compareAtPrice, ratingAverage, ratingCount, stock, contents], ii) => {
...
function contentsFor(slug) {
  return CONTENTS.get(slug) ?? [];
}
function setContents(slug, items) {
...
var HERO_SLUG = "premium-sawan-curation";
var HERO_COPY = {
  shortDescription: "Seven pieces \u2014 green chudi, a handmade resin pendant and earrings, mehawar, nail paint, a premium scrunchie and a wishes card \u2014 set in a rigid box, lined, ribboned and sealed by hand.",
  description: "Our best-selling Sawan curation. Each box is packed to order: the resin pieces are poured and cured in small batches, the bangles matched by shade, and the lid foiled before the satin ribbon goes on. It ships gift-ready \u2014 nothing to wrap, nothing to explain."
};
function detailFor(entry) {
  ...
  return __spreadProps(__spreadValues({}, entry), {
    ...
    shortDescription: hero ? HERO_COPY.shortDescription : includes.length ? `${includes.length} pieces \u2014 ${sentenceList(includes)} \u2014 set in a rigid box, lined, ribboned and sealed with a card.` : `A ${entry.categoryName.toLowerCase()} packed to order: keepsakes and treats set in a rigid box, lined, ribboned and sealed with a card.`,
    description: hero ? HERO_COPY.description : `${entry.name} is assembled in small batches in our workshop. Every piece is checked, laid into a velvet-touch tray and covered with a gold-foiled lid, then finished with satin ribbon and a hand-written card. It arrives gift-ready \u2014 nothing to wrap, nothing to explain.`,
...
var FEATURED_SLUGS = /* @__PURE__ */ new Set([
  "premium-sawan-curation",
  "rakhi-curation-set-of-two",
  "ladies-sringar-box",
  "baby-welcome-curation",
...
var CATALOGUE_FEATURED = CATALOGUE.filter((entry) => FEATURED_SLUGS.has(entry.slug));
var REVIEW_BODIES = [
...
function facetsFor(category) {
  ...
  return {
    ...
    maxPrice: prices.length ? Math.max(...prices) : 5e3
  };
}
...
function matches(entry, query) {
  if (query.category && entry.categorySlug !== query.category)
    return false;
  if (query.search) {
  ...
  if (query.brand) {
    ...
    if (wanted.length && !wanted.includes(entry.brand))
      return false;
  }
  if (query.tags) {
    const wanted = query.tags.split(",").filter(Boolean);
    if (wanted.length && !wanted.some((tag) => entry.tags.includes(tag)))
      return false;
  }
  if (query.minPrice !== void 0 && entry.price < query.minPrice)
    return false;
  if (query.maxPrice !== void 0 && entry.price > query.maxPrice)
    return false;
  if (query.minRating !== void 0 && entry.ratingAverage < query.minRating)
    return false;
  if (query.inStock && entry.stock === 0)
    return false;
  return true;
}
...
//# sourceMappingURL=chunk-LX66BOLT.js.map
