// Every image in the catalogue is drawn here rather than fetched, so the site
// ships a consistent dark-and-gold look with no third-party photo host, no
// network round-trip and no layout shift. Each shot is an inline SVG data URI:
// a gold-foil motif on a deep green-black ground.
//
// There are fifteen motifs rather than one box, so a wall of cards reads as a
// catalogue instead of a repeat: `motifFor()` picks one from the product name,
// and the ground tint shifts underneath it.
//
// To go live with real photography, keep the call sites and change `art()` to
// return a path under public/ — e.g. `/images/curations/${slug}.jpg`. Nothing
// else in the app needs to know.

export type Motif =
  | 'box' | 'basket' | 'trunk' | 'tray' | 'thali' | 'potli' | 'jewel' | 'bangles'
  | 'candle' | 'rakhi' | 'cloth' | 'bottle' | 'mug' | 'bloom' | 'frame';

export interface ArtOptions {
  /** Small caps line under the motif — usually the occasion. */
  label: string;
  /** Second line, one notch smaller. */
  caption?: string;
  /** Shifts the ground tint so neighbouring cards don't look identical. */
  tint?: number;
  shape?: 'square' | 'portrait' | 'wide';
  /** What to draw. Defaults to the house box. */
  motif?: Motif;
}

const SIZES: Record<NonNullable<ArtOptions['shape']>, [number, number]> = {
  square: [900, 900],
  portrait: [900, 1120],
  wide: [1400, 900]
};

/** Deep grounds the gold sits on. Indexed by `tint`, wrapped. */
...
/**
 * Name → motif, first rule wins. Ordered most specific first: "Designer Rakhi
 * Trunk" is a trunk, not a rakhi, because the container is what you see.
 */
...
/** Which motif a product name should be drawn as. */
export function motifFor(name: string): Motif {
  return MOTIF_RULES.find(([pattern]) => pattern.test(name))?.[1] ?? 'box';
}

/**
 * A curation rendered as an SVG data URI.
 *
 * The motif is kept inside the middle 70% of the frame so `object-fit: cover`
 * can crop any edge — square cards, 4:5 hero, wide banner — without cutting it.
 */
export function art(options: ArtOptions): string {
...
<rect x="${unit * 0.045}" y="${unit * 0.045}" width="${w - unit * 0.09}" height="${h - unit * 0.09}"
 rx="${unit * 0.02}" fill="none" stroke="url(#f)" stroke-opacity="0.38" stroke-width="${unit * 0.0022}"/>
<ellipse cx="${cx}" cy="${cy + unit * 0.245}" rx="${unit * 0.26}" ry="${unit * 0.022}" fill="#000" opacity="0.4"/>
${MOTIFS[motif](cx, cy, unit)}
<text x="${cx}" y="${cy + unit * 0.30}" text-anchor="middle" fill="url(#f)"
...
// ---------------------------------------------------------------------------
// The motifs. Each is drawn from `u` alone so it holds its proportions at any
// size, and each stays inside y ∈ [cy − 0.30u, cy + 0.23u] — below that is the
// label, outside that is croppable margin.
// ---------------------------------------------------------------------------

/** Outline weight, shared so the whole set looks drawn by one hand. */
const line = (u: number) => u * 0.0042;

/** Common attribute runs, kept short because they repeat in every motif. */
const draw = (u: number, width = 1) =>
  `fill="none" stroke="url(#f)" stroke-width="${line(u) * width}" stroke-linecap="round" stroke-linejoin="round"`;
const wash = (u: number, opacity = 0.12, width = 1) =>
  `fill="url(#f)" fill-opacity="${opacity}" stroke="url(#f)" stroke-width="${line(u) * width}" stroke-linejoin="round"`;
const solid = (opacity = 0.85) => `fill="url(#f)" fill-opacity="${opacity}"`;

const MOTIFS: Record<Motif, (cx: number, cy: number, u: number) => string> = {
  /** The house mark: lidded box, ribbon and a bow. */
  box: (cx, cy, u) => {
    const bw = u * 0.40, bh = u * 0.27, by = cy - u * 0.04;
    const lw = bw * 1.12, lh = u * 0.075, ly = by - lh;
    const rw = u * 0.045, bow = u * 0.062;
    return `
<rect x="${cx - bw / 2}" y="${by}" width="${bw}" height="${bh}" rx="${u * 0.012}" ${wash(u, 0.10)}/>
...
<circle cx="${cx}" cy="${ly - u * 0.004}" r="${u * 0.014}" ${solid(1)}/>`;
  },

  /** Woven basket with an arched handle and something peeking over the rim. */
  basket: (cx, cy, u) => `
<path d="M${cx - u * 0.175} ${cy - u * 0.10} A${u * 0.175} ${u * 0.16} 0 0 1 ${cx + u * 0.175} ${cy - u * 0.10}" ${draw(u, 1.2)}/>
<circle cx="${cx - u * 0.075}" cy="${cy - u * 0.135}" r="${u * 0.038}" ${wash(u, 0.28)}/>
<circle cx="${cx + u * 0.055}" cy="${cy - u * 0.15}" r="${u * 0.03}" ${wash(u, 0.2)}/>
<path d="M${cx - u * 0.225} ${cy - u * 0.055} L${cx + u * 0.225} ${cy - u * 0.055} L${cx + u * 0.165} ${cy + u * 0.20} L${cx - u * 0.165} ${cy + u * 0.20}Z" ${wash(u, 0.10)}/>
<rect x="${cx - u * 0.245}" y="${cy - u * 0.10}" width="${u * 0.49}" height="${u * 0.05}" rx="${u * 0.012}" ${wash(u, 0.2)}/>
...
<path d="M${cx - u * 0.10} ${cy - u * 0.05} L${cx - u * 0.075} ${cy + u * 0.20}
 M${cx} ${cy - u * 0.05} L${cx} ${cy + u * 0.20}
 M${cx + u * 0.10} ${cy - u * 0.05} L${cx + u * 0.075} ${cy + u * 0.20}" ${draw(u, 0.7)} stroke-opacity="0.6"/>`,
...
<rect x="${cx - u * 0.22}" y="${cy - u * 0.02}" width="${u * 0.44}" height="${u * 0.21}" rx="${u * 0.014}" ${wash(u, 0.10)}/>
...
<rect x="${cx - u * 0.035}" y="${cy - u * 0.04}" width="${u * 0.07}" height="${u * 0.075}" rx="${u * 0.012}" ${solid(0.9)}/>`,
  ...
  tray: (cx, cy, u) => `
<path d="M${cx - u * 0.25} ${cy + u * 0.05} L${cx - u * 0.25} ${cy + u * 0.10} A${u * 0.25} ${u * 0.09} 0 0 0 ${cx + u * 0.25} ${cy + u * 0.10} L${cx + u * 0.25} ${cy + u * 0.05}Z" ${wash(u, 0.14)}/>
<ellipse cx="${cx}" cy="${cy + u * 0.05}" rx="${u * 0.25}" ry="${u * 0.09}" ${wash(u, 0.08, 1.2)}/>
<ellipse cx="${cx}" cy="${cy + u * 0.05}" rx="${u * 0.20}" ry="${u * 0.068}" ${draw(u, 0.7)} stroke-opacity="0.55"/>
<circle cx="${cx - u * 0.10}" cy="${cy + u * 0.03}" r="${u * 0.035}" ${wash(u, 0.3)}/>
<circle cx="${cx + u * 0.005}" cy="${cy + u * 0.055}" r="${u * 0.028}" ${wash(u, 0.22)}/>
...
<path d="M${cx} ${cy - u * 0.055} C${cx - u * 0.032} ${cy - u * 0.10}, ${cx - u * 0.014} ${cy - u * 0.145}, ${cx} ${cy - u * 0.175}
 C${cx + u * 0.014} ${cy - u * 0.145}, ${cx + u * 0.032} ${cy - u * 0.10}, ${cx} ${cy - u * 0.055}Z" ${solid(0.9)}/>`,
...
<path d="M${cx - u * 0.05} ${cy - u * 0.10} C${cx - u * 0.035} ${cy - u * 0.175}, ${cx - u * 0.10} ${cy - u * 0.185}, ${cx - u * 0.115} ${cy - u * 0.135}
 M${cx + u * 0.05} ${cy - u * 0.10} C${cx + u * 0.035} ${cy - u * 0.175}, ${cx + u * 0.10} ${cy - u * 0.185}, ${cx + u * 0.115} ${cy - u * 0.135}" ${draw(u, 1)}/>
<path d="M${cx - u * 0.10} ${cy + u * 0.05} C${cx - u * 0.06} ${cy + u * 0.13}, ${cx + u * 0.06} ${cy + u * 0.13}, ${cx + u * 0.10} ${cy + u * 0.05}" ${draw(u, 0.7)} stroke-opacity="0.55"/>`,
...
<circle cx="${cx + u * 0.19}" cy="${cy - u * 0.145}" r="${u * 0.017}" ${solid(0.9)}/>
<path d="M${cx} ${cy + u * 0.155} L${cx - u * 0.055} ${cy + u * 0.06} L${cx} ${cy + u * 0.005} L${cx + u * 0.055} ${cy + u * 0.06}Z" ${wash(u, 0.45, 1.1)}/>
...
<path d="M${cx} ${cy - u * 0.095} C${cx - u * 0.042} ${cy - u * 0.15}, ${cx - u * 0.018} ${cy - u * 0.20}, ${cx} ${cy - u * 0.245}
 C${cx + u * 0.018} ${cy - u * 0.20}, ${cx + u * 0.042} ${cy - u * 0.15}, ${cx} ${cy - u * 0.095}Z" ${solid(0.9)}/>
...
<path d="M${cx + u * 0.17} ${cy + u * 0.03} C${cx + u * 0.146} ${cy - u * 0.005}, ${cx + u * 0.16} ${cy - u * 0.035}, ${cx + u * 0.17} ${cy - u * 0.062}
 C${cx + u * 0.18} ${cy - u * 0.035}, ${cx + u * 0.194} ${cy - u * 0.005}, ${cx + u * 0.17} ${cy + u * 0.03}Z" ${solid(0.85)}/>`,
...
<path d="M${cx - u * 0.13} ${cy + u * 0.045} C${cx - u * 0.21} ${cy + u * 0.115}, ${cx - u * 0.15} ${cy + u * 0.185}, ${cx - u * 0.225} ${cy + u * 0.215}
 M${cx + u * 0.13} ${cy + u * 0.045} C${cx + u * 0.21} ${cy + u * 0.115}, ${cx + u * 0.15} ${cy + u * 0.185}, ${cx + u * 0.225} ${cy + u * 0.215}" ${draw(u, 1.1)}/>`;
  ...
  cloth: (cx, cy, u) => `
<rect x="${cx - u * 0.175}" y="${cy - u * 0.115}" width="${u * 0.35}" height="${u * 0.085}" rx="${u * 0.016}" ${wash(u, 0.20)}/>
...
<path d="M${cx - u * 0.09} ${cy - u * 0.115} L${cx - u * 0.09} ${cy - u * 0.03}
 ...
 M${cx - u * 0.10} ${cy - u * 0.015} L${cx - u * 0.10} ${cy + u * 0.07}
 M${cx + u * 0.10} ${cy - u * 0.015} L${cx + u * 0.10} ${cy + u * 0.07}" ${draw(u, 0.6)} stroke-opacity="0.5"/>
...
<rect x="${cx - u * 0.032}" y="${cy - u * 0.10}" width="${u * 0.064}" height="${u * 0.085}" rx="${u * 0.008}" ${wash(u, 0.18)}/>
...
<ellipse cx="${cx - u * 0.005}" cy="${cy - u * 0.045}" rx="${u * 0.12}" ry="${u * 0.032}" ${wash(u, 0.28)}/>
<path d="M${cx - u * 0.055} ${cy - u * 0.085} C${cx - u * 0.09} ${cy - u * 0.125}, ${cx - u * 0.02} ${cy - u * 0.15}, ${cx - u * 0.055} ${cy - u * 0.195}
 M${cx + u * 0.045} ${cy - u * 0.085} C${cx + u * 0.01} ${cy - u * 0.125}, ${cx + u * 0.08} ${cy - u * 0.15}, ${cx + u * 0.045} ${cy - u * 0.195}" ${draw(u, 0.9)} stroke-opacity="0.65"/>`,
  ...
  /** A wrapped posy. */
  bloom: (cx, cy, u) => `
<path d="M${cx - u * 0.115} ${cy + u * 0.20} L${cx - u * 0.06} ${cy + u * 0.03} L${cx + u * 0.06} ${cy + u * 0.03} L${cx + u * 0.115} ${cy + u * 0.20}Z" ${wash(u, 0.14)}/>
<path d="M${cx - u * 0.075} ${cy + u * 0.085} L${cx + u * 0.075} ${cy + u * 0.085}" ${draw(u, 1.2)}/>
<path d="M${cx - u * 0.03} ${cy + u * 0.03} L${cx - u * 0.075} ${cy - u * 0.075}
 M${cx} ${cy + u * 0.03} L${cx} ${cy - u * 0.10}
 M${cx + u * 0.03} ${cy + u * 0.03} L${cx + u * 0.075} ${cy - u * 0.075}" ${draw(u, 0.8)} stroke-opacity="0.6"/>
<circle cx="${cx - u * 0.085}" cy="${cy - u * 0.10}" r="${u * 0.05}" ${wash(u, 0.3)}/>
...
<circle cx="${cx}" cy="${cy - u * 0.135}" r="${u * 0.022}" ${solid(0.9)}/>`,
  ...
  frame: (cx, cy, u) => `
<rect x="${cx - u * 0.165}" y="${cy - u * 0.165}" width="${u * 0.33}" height="${u * 0.30}" rx="${u * 0.016}" ${wash(u, 0.10, 1.2)}/>
<rect x="${cx - u * 0.115}" y="${cy - u * 0.115}" width="${u * 0.23}" height="${u * 0.20}" rx="${u * 0.008}" ${draw(u, 0.8)} stroke-opacity="0.6"/>
<path d="M${cx - u * 0.115} ${cy + u * 0.085} L${cx - u * 0.035} ${cy - u * 0.02} L${cx + u * 0.02} ${cy + u * 0.04} L${cx + u * 0.065} ${cy - u * 0.005} L${cx + u * 0.115} ${cy + u * 0.085}Z" ${wash(u, 0.28, 0.7)}/>
<circle cx="${cx + u * 0.058}" cy="${cy - u * 0.072}" r="${u * 0.022}" ${solid(0.7)}/>
<path d="M${cx + u * 0.055} ${cy + u * 0.135} L${cx + u * 0.13} ${cy + u * 0.205}" ${draw(u, 1.2)}/>`
};

/** Used wherever a product has no image of its own. */
export const FALLBACK_SHOT = art({ label: 'Anuvesha & Co.', caption: 'GIFT CURATION' });
